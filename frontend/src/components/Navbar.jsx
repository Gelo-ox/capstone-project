        ...
        <nav className='bg-[#FFF6F6] shadow-sm  py-2 px-5 flex justify-end'>
            <div className='flex items-center gap-10'>
                <div onClick={() => setNotifClicked(!notifClicked)} className='relative cursor-point'>
                    ...
                    <div className='absolute -top-3 left-3 bg-red-500 p-2 w-5 h-5 text-white font-medium text-[10px] rounded-full flex justify-center items-center'>
                        10
                    </div>
                    <div className={`absolute top-10 right-0 w-sm h-150 bg-white shadow-xl z-10 rounded-2xl overflow-auto ${notifClicked ? "block" : "hidden"}`}>
                        <h2 className='p-5 text-xl font-medium'>Notifications</h2>
                    </div>
                </div>
                <div onClick={() => setIsClicked(!isClicked)} className='p-4 bg-gray-200 rounded-full hover:cursor-pointer relative'>
